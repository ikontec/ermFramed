...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST)) {
    ...
    // Get students
    $student_result = $conn->query("SELECT DISTINCT s.student_id, s.first_name, s.middle_name, s.last_name FROM students s JOIN class_marks c ON s.student_id = c.student_id WHERE c.exam_id = $exam_id AND c.class = '$class'");
    ...
    while ($student_row = $student_result->fetch_assoc()) {
        ...
        $subject_query = $conn->query("SELECT UPPER(subject_name) AS subject_name FROM class_marks WHERE student_id = '$student_id' AND exam_id = $exam_id AND class = '$class'");
        $registered_subjects = [];
...
<body>
    ...
    <div id="loadingWrapper" class="loading-wrapper" style="display: none;">
        <img id="loadingImage" src="../../images/sms.GIF" alt="Sending...">
        <div id="loadingMessage">Sending emails... Please wait.</div>
    </div>
    
    <!-- Form to send the results -->
    <form action="send_emails_async.php" method="POST" onsubmit="showLoadingStatus()">
        <!-- Hidden inputs to pass exam_id and class to the PHP script -->
        ...
        <!-- Submit button to trigger the sending of results -->
        <button type="submit" id="sendResultsButton">Send Results to Parents</button>
    </form>
    ...
    <script>
        // Function to show the loading message and image when the form is submitted
        function showLoadingStatus() {
            document.getElementById("loadingWrapper").style.display = 'flex';  // Show the loading message and image
            document.getElementById("sendResultsButton").disabled = true;      // Disable the submit button to prevent multiple submissions
        }
    </script>
